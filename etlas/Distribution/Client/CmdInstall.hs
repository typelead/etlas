...
--   files instead of symlinks. This means that 'install'ing libraries
...
installAction (configFlags, configExFlags, installFlags, haddockFlags, newInstallFlags)
  ...
  let
    withProject = do
      ...
      pkgDb <- projectConfigWithBuilderRepoContext verbosity'
                (buildSettings localBaseCtx) (flip (getSourcePackages verbosity) binariesPath)
  ...
  (specs, selectors, config) <- withProjectOrGlobalConfig verbosity globalConfigFlag (globalSendMetrics globalFlags)
